...
import { buildStreetHotspots, type StreetHotspot } from '@/utils/streetHotspots'

interface StreetHotspotsSummaryProps {
  events: BikeEvent[]
...
export default function StreetHotspotsSummary({
  events,
  selectedStreet,
  onSelectStreet,
  onSelectEvent,
}: StreetHotspotsSummaryProps) {
  const { hotspots, excludedNoStreetCount } = useMemo(() => buildStreetHotspots(events), [events])

  const selected = useMemo<StreetHotspot | null>(() => {
    if (!selectedStreet) return null
    return hotspots.find((h) => h.street === selectedStreet) ?? null
  }, [hotspots, selectedStreet])

  const top10 = useMemo(() => hotspots.slice(0, 10), [hotspots])
  const top25 = useMemo(() => hotspots.slice(0, 25), [hotspots])

  const top10Counts = useMemo(
    () =>
      top10.map((h) => ({
        street: h.street.length > 18 ? h.street.slice(0, 18) + '...' : h.street,
        fullStreet: h.street,
        total: h.totalCount,
      })),
    [top10]
        ...
        <div className="flex items-start justify-between gap-6">
          <div>
            ...
            <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
              {excludedNoStreetCount.toLocaleString()} events have no street label and are excluded
              from street hotspots.
            </p>
          </div>
          <div className="text-xs text-gray-500 dark:text-gray-400">
            {hotspots.length.toLocaleString()} streets
          </div>
        </div>
      </div>
      ...
      {selected && (
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
          <div className="flex items-start justify-between gap-6">
            ...
            <button
              onClick={() => onSelectStreet(selected.street, selected.center)}
              ...
              title="Center map on this street"
            >
              Center on map
            </button>
          </div>
          ...
          <div className="mt-4 grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2">
              <div className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-2">
                Most recent events on this street
              </div>
              ...
              <BarChart data={top10Counts} margin={{ left: 8, right: 8, top: 10, bottom: 10 }}>
                ...
                <Tooltip
                  ...
                  formatter={(value: number | undefined, _name: string | undefined, props: any) => [
                    value,
                    props.payload.fullStreet || props.payload.street,
                  ]}
                />
              ...
              <BarChart data={top10Status} margin={{ left: 8, right: 8, top: 10, bottom: 10 }}>
                ...
                <Tooltip
                  ...
                  labelFormatter={(label: any) => {
                    const row = top10Status.find((r) => r.street === label)
                    return row?.fullStreet || String(label)
                  }}
                />
              ...
              {top25.map((row) => (
                <tr
                  ...
                  className={
                    ...
                    (selectedStreet === row.street ? 'bg-gray-50 dark:bg-gray-700' : '')
                  }
                  onClick={() => onSelectStreet(row.street, row.center)}
                  title="Center map and show street details"
                >
