...
class RestrictedPostgresRunner(PostgresRunner):
    ...
    def _validate_sql(self, sql: str) -> tuple[bool, str]:
        ...
        """
        # Enforce read-only analytics queries.
        # This runner is meant for exploration; block DDL/DML even on allowed tables/views.
        sql_start = (sql or "").lstrip()
        if not sql_start:
            return False, "Empty SQL query is not allowed."

        first_token_match = re.match(r"([a-zA-Z_]+)", sql_start)
        ...
        # Reject common DDL/DML keywords anywhere in the query body.
        if re.search(
        ...
        # Reject multiple statements (very conservative).
        if ";" in sql_cleaned.strip().rstrip(";"):
